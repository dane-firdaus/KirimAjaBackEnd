...
use App\ServiceType;
use App\CityBranchOffice; // 20220823 - ABI
...
class BookingController extends Controller
{
    ...
    protected $comission_sc;
    protected $comission_dp10;
    protected $comission_sc10;
    protected $dpp;
    protected $ppn;
    ...
    public function __construct(Client $client)
    {
        ...
        $this->comission_sc = $Commission->subconsole;
        $this->comission_dp10 = $Commission->droppoint_10;
        $this->comission_sc10 = $Commission->subconsole_10;
        $this->dpp = $Commission->dpp;
        $this->ppn = $Commission->ppn;
    }
    ...
    private function newBooking(Request $request)
    {
        ...
        if (!ctype_digit($parameter['origin']['phone'])) {
            return response()->json(['message' => 'Harap Masukan No. Telp dengan benar'], 406);
        }

        if (!ctype_digit($parameter['destination']['phone'])) {
            return response()->json(['message' => 'Harap Masukan No. Telp dengan benar'], 406);
        }
        ...
        // $booking->booking_destination_zip = 0;
        ...
        // // cek code cityname T10 20212210
        // $request = $this->client->get('http://service-dev.goexpress.id/rss/json/hubcity/'.$hub);

        //Phase#2Week#1 - Bayu - START
        $pickupStatus = false;
        if ($request->has('pickupStatus')) {
            $pickupStatus = $parameter['pickupStatus'];
            $booking->pickup_status = $parameter['pickupStatus'];
            $booking->schedule_date = $parameter['scheduleDate'];
            $booking->schedule_time = $parameter['scheduleTime'];
        }

        $service = 'SLV';
        if ($request->has('service')) {
            $service = $parameter['service'];
            $booking->service_type = $parameter['service'];
        }

        if (!$pickupStatus && $service == 'PLT') {
            return response()->json(['message' => 'Layanan Same Day Hanya dapat dilakukan menggunakan Pickup Paket'], 406);
        }

        if ($request->has('campaign')) {
            if ($user->campaign == "" || $user->campaign == null) {
                $campaign = Campaign::Where('deeplink_param', $parameter['campaign'])->first();
                if (!is_null($campaign)) {
                    if ($campaign->unit_creator == "External") {
                        $booking->campaign = $parameter['campaign'];
                    } else {
                        $temp_booking = Booking::Where('user_id', $user->id)->where('campaign', $parameter['campaign'])->get();
                        if (sizeof($temp_booking) < 1) {
                            $booking->campaign = $parameter['campaign'];
                        }
                    }
                }
        ...
        if ($request->has('platform')) {
            $booking->platform = $parameter['platform'];
            // if($booking->platform == 'ios'){
            //     return response()->json(['message' => 'Maaf Aplikasi sedang dalam masa maintenance, mohon menunggu update aplikasi terbaru Kami, atau Kamu bisa megakses website Kami di kirimaja.id'], 406);
            // }
        } else {
            return response()->json(['message' => 'Silakan Update Aplikasi Terbaru Kami untuk dapat melanjutkan'], 406);
        }

        if ($pickupStatus) {
            $booking->booking_delivery_point_id = 0;
            $deliveryPointType = 'pickup';

            if ($booking->pickup_status) {
                $ajcController = new AJCController();
                $kecamatan = $ajcController->kecamatanPickup($parameter['origin']['city']);
                if (sizeof($kecamatan) <= 0) {
                    return response()->json(['message' => 'Kecamatan tidak ter-cover layanan pick-up'], 406);
                }
            }
        } else {
        ...
        //Phase#2Week#1 - Bayu - START
        if ($deliveryPointType == 'droppoint' || $deliveryPointType == 'pickup') {
            $booking->valid = true;
        }
        //Phase#2Week#1 - Bayu - END
        ...
        if ($deliveryPointType == "droppoint") {
            if ($totalWeight > 10) {
                // $comission = 25;
                $comission = $this->comission_dp10;
            } else {
                // $comission = 50;
                $comission = $this->comission_dp;
            }
            ...
            if (auth()->user()->user_type == 'customer') {
                ...
                // }
                \DB::rollBack();
                return response()->json(['message' => 'Illegal User Type'], 406);
            } else {
                if ($totalWeight > 10) {
                    // $comission = 15;
                    // $subConsoleCommission = 10;
                    $comission = $this->comission_sc10;
                    $subConsoleCommission = $this->comission_dp10 - $this->comission_sc10;
                } else {
                    // $comission = 40;
                    // $subConsoleCommission = 10;
                    $comission = $this->comission_sc;
                    $subConsoleCommission = $this->comission_dp - $this->comission_sc;
                }
            }
            //Phase#2Week#1 - Bayu - START
        } else if ($deliveryPointType == "pickup") {
            $comission = 0;
        }
        //Phase#2Week#1 - Bayu - END
        ...
        if ($request->has('promoCode') && $request->input('promoCode') != '') {
            ...
            if (is_null($voucher)) {
                ...
                if ($voucher->voucher_type == 'percentage') {
                    ...
                    if ($voucher->max_percentage_amount != null && $voucher->max_percentage_amount > 0) {
                        if ($voucherValue > $voucher->max_percentage_amount) {
                            $voucherValue = $voucher->max_percentage_amount;
                        }
                    }
                }

                $validatePromo = ($paymentController->validatePromo($promoCode, $pickupStatus, $voucherValue));
        ...
        if (auth()->user()->user_type == 'customer') {
            $nilaiDpp = $totalCost * $this->dpp / 100;
            $nilaiPpn = floor($nilaiDpp * $this->ppn / 100);
            $payment->transaction_tax = number_format($nilaiPpn, 2, '.', '');
    ...
    public function myBooking()
    {
        ...
        if (strpos($headers['User-Agent'], 'com.garuda-indonesia.GA-DTD') && strpos($headers['User-Agent'], 'build:34')) {
            $bookings = Booking::with(['details.commodity:id,commodity_name', 'payment', 'deliveryPoint', 'subConsole:id,fullname,address,kecamatan,phone', 'subConsole.identityCard:id,user_id,profile_image', 'shipment', 'exceed', 'service'])
            ...
            $bookings = Booking::with(['details.commodity:id,commodity_name', 'payment', 'deliveryPoint', 'subConsole:id,fullname,address,kecamatan,phone', 'subConsole.identityCard:id,user_id,profile_image', 'shipment', 'exceed', 'cart', 'service'])
                ->where(['user_id' => auth()->user()->id])
                ->orderBy('id', 'desc')
                ->paginate(10);
            //20210323 - TID: U9LgjemB - KIBAR
    ...
    // Phase#2Week#1 - Bayu - START
    public function myBookingPickup()
    {
        // if (auth()->user()->user_type == 'customer') {
        ...
        if (strpos($headers['User-Agent'], 'com.garuda-indonesia.GA-DTD') && strpos($headers['User-Agent'], 'build:34')) {
            $bookings = Booking::with(['details.commodity:id,commodity_name', 'payment', 'deliveryPoint', 'subConsole:id,fullname,address,kecamatan,phone', 'subConsole.identityCard:id,user_id,profile_image', 'shipment', 'exceed', 'service'])
                ...
                ->where(function ($query) {
                    $query
                        ->where('pickedup_date', '>', date('Y-m-d H:i:s', strtotime('-24 hours')))
                        ->orWhere('pickedup_date', null);
                })
                ->orderBy('id', 'desc')
                ->paginate(50);
        } else if (array_key_exists('Ka-Agent', $headers) && !is_null($headers['Ka-Agent'])) {
    ...
    public function findMyBooking(Request $request)
    {
        ...
        $queryWhere = "UPPER(booking_code) = ? AND user_id = ?";
        $paramWhere = array(
            strtoupper($request->input('bookingCode')),
            auth()->user()->id
        );
        if ($request->has('pickupStatus')) {
            $queryWhere = "UPPER(booking_code) = ? AND user_id = ? AND pickup_status = ?";
            $paramWhere = array(
                strtoupper($request->input('bookingCode')),
                auth()->user()->id,
                $request->input('pickupStatus')
            );
        }
        ...
        if (array_key_exists('Ka-Agent', $headers) && !is_null($headers['Ka-Agent'])) {
            ...
            //20210323 - TID: U9LgjemB - KIBAR
            $bookings = Booking::with(['details', 'payment', 'deliveryPoint', 'subConsole:id,fullname,address,kecamatan,phone', 'shipment', 'exceed', 'cart', 'service'])
                // ->whereRaw(
                //     'UPPER(booking_code) = ? AND user_id = ?', [strtoupper($request->input('bookingCode')), auth()->user()->id]
                // )
                ->whereRaw($queryWhere, $paramWhere)
                ->orderBy('id', 'desc')
                ->paginate(50);
            //20210323 - TID: U9LgjemB - KIBAR
        } else {
            $bookings = Booking::with(['details', 'payment', 'deliveryPoint', 'subConsole:id,fullname,address,kecamatan,phone', 'shipment', 'exceed', 'service'])
                // ->whereRaw(
                //     'UPPER(booking_code) = ? AND user_id = ?', [strtoupper($request->input('bookingCode')), auth()->user()->id]
                // )
                ->whereRaw($queryWhere, $paramWhere)
    ...
    public function findMyBookingPickup(Request $request)
    {
        $headers = getallheaders();
    ...
    public function deleteBooking(Request $request)
    {
        if ($request->has('option') && $request->input('option') == 'delete') {
            $booking = Booking::with(['payment'])->whereRaw(
                'UPPER(booking_code) = ? AND user_id = ?',
                [strtoupper($request->input('bookingCode')), auth()->user()->id]
            )->first();
    ...
    public function getBooking(Request $request)
    {
        ...
        if (!is_null($voucherInfo)) {
            ...
            $booking->voucher_max = $voucherInfo->master->max_percentage_amount;
        }

        //20210224 - TID: 3B23WByr - END
        //20220823 - ABI add leadtime - START
        $kota = $booking->booking_origin_city;
        $kota_kab = explode(", ", $kota)[1];
        $cityBranch = CityBranchOffice::where('city_name', $kota_kab)->first();
        $origin = $cityBranch->region_area;
        $destination = $booking->booking_destination_city;
        ...
        $requestSIS = $this->client->post(config('global.LINK_API_SIS').'/trace/pricelist', $param);
        $response = $requestSIS ? $requestSIS->getBody()->getContents() : null;
        if ($response != null) {
            $response = json_decode($response, true);
            if (array_key_exists('list', $response)) {
                if (!empty($response['list'])) {
                    $price = (int)$response['list'][0]['Price1st'];

                    $leadTime = $response['list'][0]['LeadTime'];
                    $leadTimeText = $leadTime." hari";
                    if($leadTime > 1){
                        $leadTime2 = $leadTime - 1;
                        $leadTimeText = $leadTime2."-".$leadTime." hari";
                    }
                }
            }
        }
        $service['price'] = $price;
        $service['leadtime'] = $leadTimeText;
        $booking->leadtime = $leadTimeText;
        //20220823 - ABI add leadtime - END
    ...
    public function editBooking(Request $request)
    {
        ...
        if ($request->has('deliveryPoint')) {
            ...
            $updatedCommission = $this->comission_dp;
            ...
            if (is_null($subConsole)) {
                ...
                $updatedCommission = $this->comission_sc;
            ...
            if ($getShipment > 10) {
                if (!is_null($checkDPoint)) {
                    ...
                    $updatedCommission = $this->comission_dp10;
                } else if (!is_null($checkSubcon)) {
                    // $updatedCommission = 15;
                    $updatedCommission = $this->comission_sc10;
                }
            }
            ...
            if (!is_null($trxSubconsole)) {
                ...
                if (!$bookingValid) {
                    ...
                    if ($booking->user->user_type == 'customer') {
                        ...
                        $trxSubconsole->transaction_comission_amount = $totalCost * 40 / 100;
                        $trxSubconsole->transaction_comission_by = 40;
                        \DB::rollBack();
                        return response()->json(['message' => 'Illegal User Type.'], 406);
                    } else {
                        $selisih_comission = 0;
                        if ($getShipment > 10) {
                            $selisih_comission = $this->comission_dp10 - $this->comission_sc10;
                        } else {
                            $selisih_comission = $this->comission_dp - $this->comission_sc;
                        }
                        // $trxSubconsole->transaction_comission_amount = $totalCost*10/100;
                        // $trxSubconsole->transaction_comission_by = 10;
                        $trxSubconsole->transaction_comission_amount = $totalCost * $selisih_comission / 100;
                        $trxSubconsole->transaction_comission_by = $selisih_comission;
                    }
                    $trxSubconsole->valid_status = 'order';
                    $trxSubconsole->save();
        ...
        if ($request->has('shipment')) {
            ...
            for ($i = 0; $i < count($shipments); $i++) {
                ...
                if (!is_null($package)) {
                    ...
                    //20210602 - TID: uhRBUCiI - START
                    if (array_key_exists('weight', $shipments[$i])) {
                        $package->package_actual_weight = $shipments[$i]['weight'];
                        $package->package_actual_volume = (($shipments[$i]['length'] * 1) * ($shipments[$i]['width'] * 1) * ($shipments[$i]['height'] * 1)) / 6000;
                    }
                    //20210602 - TID: uhRBUCiI - END
                    $package->save();
            ...
            $nilaiDpp = $totalCost * $this->dpp / 100;
            $nilaiPpn = floor($nilaiDpp * $this->ppn / 100);
            $booking->payment->transaction_tax = number_format($nilaiPpn, 2, '.', '');
        ...
        if ($request->has('scheduleDate')) {
            ...
            if ($request->has('scheduleTime')) {
                if ($booking->pickup_status) {
                    $booking->schedule_date = $request->input('scheduleDate');
                    $booking->schedule_time = $request->input('scheduleTime');
                    $booking->save();

                    \DB::commit();

                    return response()->json(['message' => 'Pick-up schedule updated'], 201);
                } else {
                    return response()->json([
                        'message' => 'Anda tidak dapat merubah data ini'
                    ], 200);
                }
            } else {
                return response()->json([
        ...
        if (auth()->user()->user_type != 'user') {
            ...
            if ($subConsoleTransaction != null) {
                if ($booking->user->user_type == 'customer') {
                    ...
                    $subConsoleTransaction->transaction_comission_amount = $totalCost * $selisih_comission / 100;
                }
                $subConsoleTransaction->save();
            }
    ...
    public function getPriceService(Request $request)
    {
                    ...
                    if (!empty($response['list'])) {
                        if ($type == 'droppoint') {
                            if ($estWeight != null && $estWeight > 10) {
                                $response['list'][0]['Commission'] = $this->comission_dp10;
                            } else {
                                $response['list'][0]['Commission'] = $this->comission_dp;
                            }
                        } else if ($type == 'subconsole') {
                            if ($estWeight != null && $estWeight > 10) {
                                $response['list'][0]['Commission'] = $this->comission_sc10;
                            } else {
                                $response['list'][0]['Commission'] = $this->comission_sc;
                            }
                        } else if ($type == 'pickup') {
                            $response['list'][0]['Commission'] = 0;
                        }
                        if ($type != 'pickup' && $service->alias == 'PLT') {
                            $price = array();
                        } else {
                            $price = $response['list'];
                        }
        ...
        return response($hasil, 200, ['content-type' => 'application/json']);
    }
    ...
    //     //Phase#2Week#1 - Bayu - END
    //
    //     if($type != 'pickup' && $service == 'PLT'){
    //         return response()->json(['message' => 'Mohon maaf Kak, untuk pengiriman "Same Day" hanya bisa dilakukan dengan layanan pickup.']);
    //     }
    //
    //     $AJCBranchCodeList = ['CGK' => 'JKT'];
    //
    ...
    //
    //     // dd($param);
    //     // return response($param, 200, ['content-type' => 'application/json']);
    //     // return response()->json(['message' => $param]);
    //
    //     //20210329 - TID: 3hZhpizs - KIBAR
    //     if ($request->has('promoCode') && $request->input('promoCode') != '')
    //     {
    //         $voucherValue = 0;$voucherMaXAmount=0;
    //         $promoCode = $request->input('promoCode');
    //         $voucher = VourcherData::where([
    //             'voucher_code' => $promoCode,
    //             //'voucher_valid' => date('Y-m-d'),//20210329 - TID: 3hZhpizs - KIBAR
    ...
    //
    //         if (is_null($voucher)) {
    //             return response()->json(['message' => 'Code Voucher tidak ditemukan'], 406);
    //         }
    //
    //         if($voucher->layanan_type != 0){
    //             if($voucher->layanan_type == 1 && $pickupStatus){
    //                 return response()->json(['message' => 'Voucher hanya bisa digunakan untuk Layanan Antar Paket']);
    //             }
    //             if($voucher->layanan_type == 2 && !$pickupStatus){
    //                 return response()->json(['message' => 'Voucher hanya bisa digunakan untuk Layanan Pick-up Paket']);
    //             }
    //         }
    //
    ...
    //                     return response()->json(['message' => "Voucher tidak bisa digunakan untuk Area Pengirim yang anda masukan!"]);
    //                 }
    //
    //             }
    //         }
    ...
    //                 // return response()->json(['message' =>'ss'.$voucher->dest_city]);
    //                 if(!empty($voucher->dest_city)){
    //                     $DestinationCode = Destination::where("name", $destination)->first();
    //                     // return response()->json(['message' => $voucher->dest_city]);
    //
    //                     if($DestinationCode){
    //                         if(!in_array($DestinationCode->code, $hubCityDes) ){
    //                             return response()->json(['message' => "Voucher  tidak bisa digunakan untuk Area Penerima yang anda masukan!"]);
    //                         }
    //                     }
    //
    //                 }
    ...
    //         }
    //
    //         // origin_hub  dest_city
    //
    //         // $CodeOrigin=(array_key_exists($branchCode, $AJCBranchCodeList)) ? $AJCBranchCodeList[$branchCode] : $branchCode;
    //         // $cityCodeDestination=$this->client->get('http://service-dev.goexpress.id/rss/json/citycodename/'.$destination);
    //         // $sim= $cityCodeDestination ? json_decode($cityCodeDestination->getBody(), true) : null;
    //         //return response()->json(['message' =>$DestinationCode->code]);
    //
    //         $paymentController = new PaymentController();
    //         $validatePromo = ($paymentController->validatePromo($promoCode, $pickupStatus));
    //
    //         if($validatePromo->status() <> 200)
    //         {
    //             return $validatePromo;
    ...
    //         $voucherMaXAmount = $voucher->max_percentage_amount == null ? 0 : $voucher->max_percentage_amount;
    //     }
    //     //20210329 - TID: 3hZhpizs - KIBAR
    //
    //     //$request = $this->client->post('http://103.9.36.120:8081/trace/pricelist', $param);
    //     $request = $this->client->post(config('global.LINK_API_SIS').'/trace/pricelist', $param);
    ...
    //                     $response['list'][0]['voucherMaXAmount'] = $voucherMaXAmount;
    //                 }
    //             }
    //             //20210329 - TID: 3hZhpizs - KIBAR
    ...
    public function getPricelist(Request $request)
    {
        ...
        return $this->getPriceSIS($destination, $dropPoint, $pickupStatus, $estWeight, $service, $promoCode);
    }

    private function getPriceSIS($destination, $dropPoint, $pickupStatus = false, $estWeight = null, $service = 'SLV', $promo = null)
    {
        if ($pickupStatus) {
            $branchCode = BranchOfficeMapping::whereRaw("TRIM(city) ILIKE :citysubcon", ['citysubcon' => '%' . $dropPoint . '%'])->first();
            if ($branchCode != null) {
        ...
        if ($promo != null) {
            ...
            $voucherMaXAmount = 0;
            $promoCode = $promo;
            $voucher = VourcherData::where(['voucher_code' => $promoCode])->first();
        ...
        if ($response != null) {
            ...
            if (array_key_exists('list', $response)) {
                ...
                $response['list'][0]['Dpp'] = $this->dpp;
                $response['list'][0]['Ppn'] = $this->ppn;
                if ($type == 'droppoint') {
                    if ($estWeight != null && $estWeight > 10) {
                        $response['list'][0]['Commission'] = $this->comission_dp10;
                    } else {
    ...
    public function getInvoice(Request $request)
    {
        ...
        //$urlConnote = "http://103.9.36.120/e-connote/".$connote."/kirimaja";
        ...
        $booking = Booking::with([
            ...
            'subConsole:id,fullname,address,kecamatan,phone',
            'subConsole.identityCard:id,user_id,profile_image',
            'validInfo:id,booking_id,valid', 'shipment', 'exceed', 'service'
        ])->where(['id' => $booking_ajc->booking_id, 'user_id' => auth()->user()->id])->first();
    ...
    {
        $mySubconsoles = SubConsoleTransaction::with([
            'booking:id,user_id,booking_code',
            'booking.user:id,fullname,phone,user_type',
            'booking.details:id,booking_id',
            'booking.shipment',
            'booking.payment',
            'deliveryPoint',
        ])
            ->where('user_id', auth()->user()->id)
            // only count data where sohib not delete order - ABI
    ...
    {
        // $dataSubconsole = SubConsoleTransaction::with(['booking:id,user_id,valid', 'deliveryPoint'])
        // ->where([
        //     'user_id' => auth()->user()->id,
        //     'valid_status' => 'order'
        // ])->get();
        ...
        // only count data where sohib not delete order - ABI
        $dataSubconsole = DB::table('trx_subconsole')
            ->join('trx_booking', 'trx_subconsole.booking_id', '=', 'trx_booking.id')
            ->whereNull('trx_booking.deleted_at')
            ->where('trx_subconsole.user_id', '=', auth()->user()->id)
            ->where('trx_subconsole.valid_status', '=', 'order')
            ->get();

        $dataCart = PaymentCart::with(['booking:id,booking_code', 'booking.payment'])->whereHas('booking.payment', function ($query) {
        ...
        $dataPickup = Booking::with(['details.commodity:id,commodity_name', 'payment', 'deliveryPoint', 'subConsole:id,fullname,address,kecamatan,phone', 'subConsole.identityCard:id,user_id,profile_image', 'shipment', 'exceed', 'cart'])
            ->whereHas('payment', function ($q) {
                $q->where('paid', true);
            })
            ->where([
        ...
        $data['cart'] = $dataCart->count();
        $data['pickup'] = $dataPickup->count();
    ...
    {
        // $request = $this->client->get('http://103.9.36.120:8081/rss/json/cabang');
        $request = $this->client->get(config('global.LINK_API_SIS') . '/rss/json/cabang');
    ...
    {
        // $request = $this->client->get('http://103.9.36.120:8081/rss/json/destination');
        $request = $this->client->get(config('global.LINK_API_SIS') . '/rss/json/destination');
    ...
    public function bookingAjc(Request $request)
    {
        ...
        // $request = $this->client->post('http://103.9.36.120:8081/trace/insertawb', $param);
        $request = $this->client->post(config('global.LINK_API_SIS') . '/trace/insertawb', $param);
