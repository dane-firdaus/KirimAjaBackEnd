...
class PaymentController extends Controller
{
    public function __construct()
    {
        $this->middleware('APITokenJWT', ['except' => [
            'getPaymentProof', 'marketplacePayment', 'redirectPayment', 'notifyPayment', 'paymentVa', 'autorunGenerateConnote']
        ]);
    }
    ...
    public function getPromo(Request $request)
    {
        ...
        //     $pickupStatus = $request->input('pickupStatus');
        // }
        return $this->validatePromo($promoCode, $pickupStatus); //20210329 - TID: 3hZhpizs - KIBAR
    }
    //20210224 - TID: 3B23WByr - END

    //20210329 - TID: 3hZhpizs - KIBAR
    public function validatePromo($promoCode, $pickupStatus, $value=0)
    {
        ...
        if (is_null($voucher)) {
            ...
            if($voucher->layanan_type != 0){
                if($voucher->layanan_type == 1 && $pickupStatus){
                    return response()->json([
                        'message' => 'Maaf, Voucher ini hanya tersedia untuk layanan Antar Paket'
                    ], 406);
                }
                if($voucher->layanan_type == 2 && !$pickupStatus){
                    return response()->json([
                        'message' => 'Maaf, Voucher ini hanya tersedia untuk layanan Pick-up Paket'
                    ], 406);
                }
            }
            ...
            return response()->json([
                ...
                'max_amount' => $voucher->max_percentage_amount == null ? 0 : $voucher->max_percentage_amount,
                'message' => 'Voucher valid'//.date('Y-m-d H:i').'|'.$voucher->start_date.'|'.$voucher->end_date,
            ], 200);
        }
    }
    //20210329 - TID: 3hZhpizs - KIBAR
            ...
            foreach ($bookings as $booking) {
                ...
                if ($booking->pickup_status){
                    ...
                    if(sizeof($kecamatan) <= 0){
                        // return 'Kecamatan Kamu tidak ter-cover layanan pick-up';
                        return view('payment_validation', [
                            'message' => 'Kecamatan Kamu tidak ter-cover layanan pick-up'
                        ]);
                    }
                    $zona = $kecamatan[0]->ZonaGroup;

                    $jadwal = $ajcController->schedulePickup($zona, $booking->schedule_date, $booking->service_type);
                    $flag_ada = false;
                    ...
                    //exit;
                    if(!$flag_ada){
                        // return 'Tidak dapat melakukan pembayaran, karena sudah melewati batas jadwal pick-up yang anda pilih, silakan merubah jadwal pick-up terlebih dahulu';
                        return view('payment_validation', [
                            'message' => 'Tidak dapat melakukan pembayaran, karena sudah melewati batas jadwal pick-up yang anda pilih, silakan merubah jadwal pick-up terlebih dahulu'
                        ]);
                    }
                }
...
//                $k=0; //payment trap connote troubleshoot case 2A
                foreach ($bookingids as $id) {
//                    if($k==0)  //payment trap connote troubleshoot case 2A
//                    { payment trap connote troubleshoot case 2A
                        $bookinInCart = PaymentCart::where('id', $id)->first();
                        $this->bookingIsPaid($bookinInCart->booking_id, $dokuResponse->input('RESPONSECODE'), $dokuResponse->input('PAYMENTCHANNEL'), $paymentReq);
//                    }  payment trap connote troubleshoot case 2A
//                    $k++; payment trap connote troubleshoot case 2A
                }
            } else {

                //payment trap connote troubleshoot case 1B
                $this->bookingIsPaid($paymentReq->booking_id, $dokuResponse->input('RESPONSECODE'), $dokuResponse->input('PAYMENTCHANNEL'), $paymentReq);
            }
            ...
            */
            ...
            if ($paymentReq->booking_id == null) {
                ...
                foreach ($cartIds as $id) {
                    ...
                    $booking = $this->bookingIsPaid($bookinInCart->booking_id, $dokuResponse->input('STATUSCODE'), $dokuResponse->input('PAYMENTCHANNEL'), $paymentReq);
                    array_push($bookings, $booking);
                }
            } else {
                $booking = $this->bookingIsPaid($paymentReq->booking_id, $dokuResponse->input('STATUSCODE'), $dokuResponse->input('PAYMENTCHANNEL'), $paymentReq);
                array_push($bookings, $booking);
    ...
    public function paymentByCoupon($bookingCode)
    {
        ...
        if (is_array($bookingCode)) {
            ...
            foreach ($bookingCode as $id) {
                $booking = $this->bookingIsPaid($id, '0000', '99', null);
                array_push($bookings, $booking);
            }
            ...
            $booking = $this->bookingIsPaid($bookingCode, '0000', '99', null);
            return view('payment_cart_redirect', [
                'paidDate' => $paidDate,
                'paidAt' => date('H:m', time()),
                'success' => true,
    ...
    public function payCart(Request $request)
    {
        ...
        foreach ($carts as $cart) {
            if ($cart->booking->pickup_status){
                ...
                if(sizeof($kecamatan) <= 0){
                    // return 'Pada cart anda terdapat Kecamatan yang tidak ter-cover layanan pick-up';
                    return view('payment_validation', [
                        'message' => 'Pada cart anda terdapat Kecamatan yang tidak ter-cover layanan pick-up'
                    ]);
                }
                $zona = $kecamatan[0]->ZonaGroup;

                $jadwal = $ajcController->schedulePickup($zona, $cart->booking->schedule_date, $cart->booking->service_type);
                $flag_ada = false;
                foreach($jadwal as $jdwl){
                ...
                //exit;
                if(!$flag_ada){
                    // return 'Tidak dapat melakukan pembayaran, karena pada cart anda terdapat booking sudah melewati batas jadwal pick-up yang anda pilih, silakan merubah jadwal pick-up terlebih dahulu';
                    return view('payment_validation', [
                        'message' => 'Tidak dapat melakukan pembayaran, karena pada cart anda terdapat booking sudah melewati batas jadwal pick-up yang anda pilih, silakan merubah jadwal pick-up terlebih dahulu'
                    ]);
                }
            }
...
//            $amount += $cart->booking->payment->transaction_tax;
    ...
    public function myCommission(Request $request)
    {
        ...
        if ($request->has('month')) {
            $transactions = Payment::with(['booking:id,booking_code,pickup_status'])->where(['user_id' => auth()->user()->id, 'paid' => true])->whereMonth('paid_at', $param['month'])->paginate(10);
            $commissionValue = Payment::with(['booking:id,booking_code'])->where(['user_id' => auth()->user()->id, 'paid' => true])->whereMonth('paid_at', $param['month'])->sum('transaction_comission_amount');
            $transactionTotal = Payment::with(['booking:id,booking_code'])->where(['user_id' => auth()->user()->id, 'paid' => true])->whereMonth('paid_at', $param['month'])->sum('transaction_total_amount');
        } elseif ($request->has('range_start') && $request->has('range_end')) {
            $transactions = Payment::with(['booking:id,booking_code,pickup_status'])->where(['user_id' => auth()->user()->id, 'paid' => true])->whereBetween('paid_at', [$param['range_start'], $param['range_end']])->paginate(10);
            $commissionValue = Payment::with(['booking:id,booking_code'])->where(['user_id' => auth()->user()->id, 'paid' => true])->whereBetween('paid_at', [$param['range_start'], $param['range_end']])->sum('transaction_comission_amount');
            $transactionTotal = Payment::with(['booking:id,booking_code'])->where(['user_id' => auth()->user()->id, 'paid' => true])->whereBetween('paid_at', [$param['range_start'], $param['range_end']])->sum('transaction_total_amount');
            if (auth()->user()->user_type == 'subconsole') {
                $subConsole = SubConsoleTransaction::with(['booking','booking.payment'])
                    ->whereHas('booking.payment', function ($query) {
                        $query->where('paid', true);
                    })
                    ->where(['user_id' => auth()->user()->id])
                    ->whereBetween('created_at', [$param['range_start'], $param['range_end']])
                    ->orderBy('id', 'desc')->paginate(10);

                $subConsoleCommissionValue = SubConsoleTransaction::with(['booking','booking.payment'])
                    ->whereHas('booking.payment', function ($query) {
                        $query->where('paid', true);
                    })
                    ->where(['user_id' => auth()->user()->id])
                    ->whereBetween('created_at', [$param['range_start'], $param['range_end']])
                    ->orderBy('id', 'desc')
                    ->sum('transaction_comission_amount');
            }
        } else {
            $transactions = Payment::with(['booking:id,booking_code,pickup_status'])->where(['user_id' => auth()->user()->id, 'paid' => true])->paginate(10);
            ...
            $transactionTotal = Payment::with(['booking:id,booking_code'])->where(['user_id' => auth()->user()->id, 'paid' => true])->sum('transaction_total_amount');
        }

        return response()->json([
            'commission' => number_format($commissionValue),
            'transactionTotal' => number_format($transactionTotal),
    ...
    public function bookingIsPaid($id, $statusCode, $channel, $paymentRequest)
    {
        ...
        $booking->payment->transaction_id = $paymentRequest->id;
    ...
    public function autorunGenerateConnote(){
        $bookingQuery = "
            SELECT
              tp.booking_id
            FROM trx_payment tp
            LEFT JOIN trx_booking tb ON tp.booking_id = tb.id
            LEFT JOIN trx_ajc_booking tab ON tb.id = tab.booking_id
            WHERE
                tp.paid = true AND
                tab.awb IS NULL
            ORDER BY tp.paid_at ASC
        ";
        $_data = \DB::select($bookingQuery);

        $msg = "No Connote Created";
        $i = 0;
        if(sizeof($_data) > 0){
            foreach($_data as $row){
                ...
                $ajcController = new AJCController();
                $gen = $ajcController->storeBooking($booking->id);

                if($gen != null){
                    $connote = $gen->detail->cnote_no;
                    $msg .= "Connote Created for Booking Code ".strtoupper($booking->booking_code)." (".$connote.") <br/>";
                    // event(new OrderReceiptEvent($booking));
                    // return response()->json($gen, 200);
                }else{
                    $msg .= "Connote Created Failed for Booking Code ".strtoupper($booking->booking_code)." <br/>";
                }
